...
export class EnvironmentRefresher {
  ...
  public static async updateWindowsTerminalProfileEnv(): Promise<void> {
    try {
      ...
      Prompt.Error(
        `Error updating terminal environment: ${error instanceof Error ? error.message : error}`,
      );
    }
  }
}
export class KeyStore {
  static globalState: vscode.ExtensionContext["globalState"] | undefined;
   ...
   */
  static getDefault<T>(key: string, defaultValue: T): T {
