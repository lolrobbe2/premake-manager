...
export class SourceUtils {
  ...
  static async register(context: vscode.ExtensionContext, path: string[]) {
    ...
    const resourcePath: string | undefined = this.getLanguageResource(
      context,
      path,
    );
    if (current.includes(resourcePath!)) {
      console.error(
        `[SOURCE_UTILS] resource was already registered in current workspace ${resourcePath}`,
      );
      return;
    }
  ...
  static async unregister(context: vscode.ExtensionContext, path: string[]) {
    const config = vscode.workspace.getConfiguration("Lua.workspace");
    const current = (config.get("library") as string[]) || [];

    const resourcePath = this.getLanguageResource(context, path);

    if (!current.includes(resourcePath!)) {
      return;
    }

    const updated = current.filter((entry) => entry !== resourcePath);
