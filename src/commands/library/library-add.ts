...
export class LibraryAddCommand extends CommandRegistrar {
    protected async execute(...args: any[]): Promise<void> {
        ...
        if (githubLink !== undefined)    
            TerminalInterface.libraryAdd(this.addPrefix(githubLink));
    }

    constructor(context: vscode.ExtensionContext, register: boolean) {
        super(context, register,'premake5.library-add',"add library");
    }
